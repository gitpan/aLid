...
B<finProtect - Script used for intercept attempts of port scanning by Fin scan.>

=head1 SYNOPSIS

B<finProtect RANGE EVT_BOUND RED_BOUND FILE_IP_BLOCKED NOTIFY FILTER [FREQ_EMAIL] >

=head1 DESCRIPTION

It is used for intercept attempts of port scanning that use the tecnic FIN Scan. The attacker must send many package TCP with purpose of obtain informations about ports. Each package has enabled only flag FIN. When is reached a particular threshold fixed in the configuration, the source ip is blocked because it's responsable of port scanning.
...
=item B<finProtect 10 5 7 /etc/aLid/ip_blocked_finprotect all drop 60> 

It executes script using a range of 10 seconds, 5 maximun attempts in the temporal window, 7 seconds as redention's threshold. Ip_blocked_finprotect is the name of the file utilized, all is the notifics typology, drop is kind of filter, and 60 is mail's frequency.
