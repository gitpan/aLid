...
B<synProtect - Script used for intercept attempts of port scanning by Syn scan.>

=head1 SYNOPSIS

B<synProtect RANGE EVT_BOUND RED_BOUND FILE_IP_BLOCKED NOTIFY FILTER [FREQ_EMAIL] >

=head1 DESCRIPTION

It is used for intercept attempts of port scanning that use the tecnic SYN Scan. The attacker must send many package TCP with purpose of obtain informations about ports. Each package has enabled only flag SYN. When is reached a particular threshold fixed in the configuration, the source ip is blocked because it's responsable of port scanning.
